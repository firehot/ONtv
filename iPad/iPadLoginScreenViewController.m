...
@interface iPadLoginScreenViewController() <UITextFieldDelegate, UITableViewDataSource, UITableViewDelegate>
...
- (void)layoutHeaderView
{
    ...
    UIImageView *ivTemp = [[UIImageView alloc] initWithFrame:CGRectMake(floorf(0.5f*(headerView.bounds.size.width - 400)), 150, 400, 90)];
    [ivTemp setImage:[UIImage imageNamed:@"img_logo"]];
    ...
    UILabel *tempLabel = [[UILabel alloc]initWithFrame:CGRectMake(15, CGRectGetMaxY(ivTemp.frame)+70, headerView.bounds.size.width-30.0f, suggestedSize.height)];
...
- (void)layoutFooterView
{
    ...
    CGFloat buttonWidth = 190.0f;//floorf(0.5f*(self.tableView.bounds.size.width-3.0f*buttonWidth));
    
    // createLoginButton
    
    UIButton *login = [UIButton buttonWithType:UIButtonTypeCustom];
    self.loginButton = login;
    self.loginButton.frame = CGRectMake(floorf(0.5f*(footerView.bounds.size.width - 400)), 30, 400, 40);
    self.loginButton.backgroundColor = [UIColor clearColor];
    self.loginButton.userInteractionEnabled = YES;
    self.loginButton.tag = 0;
    [self.loginButton setBackgroundImage:[[UIImage imageNamed:@"btn_login"] stretchableImageWithLeftCapWidth:5 topCapHeight:5] forState:UIControlStateNormal];
    ...
    self.loginButton.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin| UIViewAutoresizingFlexibleRightMargin;
    [footerView addSubview:self.loginButton];
    
    
    // createContinueButton
    continueButton = [UIButton buttonWithType:UIButtonTypeCustom];
    continueButton.frame = CGRectMake(buttonMargin, footerView.bounds.size.height-330, buttonWidth, 30); //70
    continueButton.backgroundColor = [UIColor clearColor];
    continueButton.userInteractionEnabled = YES;
    continueButton.tag = 1;
    [continueButton setBackgroundImage:[[UIImage imageNamed:@"btn_grey"] stretchableImageWithLeftCapWidth:5 topCapHeight:5] forState:UIControlStateNormal];
    ...
    continueButton.autoresizingMask = UIViewAutoresizingFlexibleRightMargin|UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleBottomMargin;
    ...
    createNewUserButton.frame = CGRectMake(footerView.bounds.size.width-buttonWidth-30.0f, footerView.bounds.size.height-330, buttonWidth, 30);
    ...
    createNewUserButton.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin|UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleBottomMargin;
...
-(void) configureLoginView
{
    ...
    TPKeyboardAvoidingTableView *tableView = [[TPKeyboardAvoidingTableView alloc] initWithFrame:(CGRectMake(floorf(0.5f*(self.view.bounds.size.width - 460)), 0, 460, self.view.bounds.size.height)) style:UITableViewStyleGrouped];
...
-(void) createNewAccountButtonClicked : (UIButton *) sender {
    ...
    if(!appDelegate.createNewAccountViewController) {
        iPadCreateNewAccountViewController *vcTemp = [[iPadCreateNewAccountViewController alloc] init];
